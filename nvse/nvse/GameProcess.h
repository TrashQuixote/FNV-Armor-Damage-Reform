...
struct ActorHitData
{
	...
	void Print() const {
		gLog.FormattedMessage(" hitLocation: %d, healthDmg: %.2f, armorDmg: %.2f,dmgMult: %.2f",
			hitLocation, healthDmg, armorDmg, dmgMult);
	}
...
struct MuzzleFlash
{
	...
	void* flashLight;	// 10 - NiPointLight
...
public:
	...
	UInt8								byte272;			// 272
	UInt8								byte273;			// 273
	tList<void>							list274;			// 274	// void == CombatTarget
	...
	void* storedAVs;			// 428 - StoredActorValues
	float								flt42C;				// 42C
	UInt32								unk430;				// 430
