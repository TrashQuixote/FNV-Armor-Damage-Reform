...
public:
	...
	enum
	{
		kFormFlags_IsMaster = 0x1,
		kFormFlags_IsAltered = 0x2,
		kFormFlags_Initialized = 0x00000008,	// set by TESForm::InitItem()
		kFormFlags_Deleted = 0x00000020,
		kFormFlags_Known = 0x40,
		kFormFlags_80 = 0x80,
		kFormFlags_DroppedRef = 0x100,
		...
		kFormFlags_StillLoading = 0x200000,
	};
...
public:
	...
	virtual int		GetBaseActorValueInt(UInt32 avCode) const;	// GetBaseActorValue (used from Eval) result in EAX
	virtual float	GetBaseActorValue(UInt32 avCode) const;		// GetBaseActorValue internal, result in st
	virtual int		GetActorValueInt(UInt32 avCode) const;		// GetActorValue internal, result in EAX
	virtual float	GetActorValue(UInt32 avCode) const;			// GetActorValue (used from Eval) result in EAX
	virtual float	GetBaseSubAV(UInt32 avCode) const;			// GetBaseActorValue04 (internal) result in st
	virtual float	GetSubAVDamage(UInt32 avCode) const;
	virtual float	GetSubAVMod(UInt32 avCode) const;			// GetDamageActorValue or GetModifiedActorValue		called from Fn_08, result in st, added to Fn_01
	virtual int		GetPermanentActorValueInt(UInt32 avCode) const;		// Manipulate GetPermanentActorValue, maybe convert to integer.
	virtual float	GetPermanentActorValue(UInt32 avCode) const;		// GetPermanentActorValue (used from Eval) result in EAX
	virtual Actor* GetActor() const;									// GetActorBase (= this - 0x100) or GetActorBase (= this - 0x0A4)
	virtual UInt16	GetLevel() const;									// GetLevel (from ActorBase)

	// SkillsCurrentValue[14] at index 20
	__forceinline float GetThresholdedAV(UInt32 avCode)
	{
		return ThisStdCall<float>(0x66EF50, this, avCode);
	}
};
...
public:
	...
	UInt32			objectiveId;	// 004 Objective FormulaIndex in the GECK
