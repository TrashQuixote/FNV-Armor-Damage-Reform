...
template <typename T_Ret = UInt32, typename ...InputArgs>
__forceinline T_Ret ThisStdCall(UInt32 _addr, const void *_this, InputArgs ...args)
{
	return ((T_Ret (__thiscall *)(const void*, InputArgs...))_addr)(_this, std::forward<InputArgs>(args)...);
}

template <typename T_Ret = void, typename ...InputArgs>
__forceinline T_Ret StdCall(UInt32 _addr, InputArgs ...args)
{
	return ((T_Ret (__stdcall *)(InputArgs...))_addr)(std::forward<InputArgs>(args)...);
}

template <typename T_Ret = void, typename ...InputArgs>
__forceinline T_Ret CdeclCall(UInt32 _addr, InputArgs ...args)
{
	return ((T_Ret (__cdecl *)(InputArgs...))_addr)(std::forward<InputArgs>(args)...);
}
...
template <typename T, const UInt32 ConstructorPtr = 0, typename... InputArgs>
T* New(InputArgs &&... args)
{
	auto* alloc = FormHeap_Allocate(sizeof(T));
	if constexpr (ConstructorPtr)
	{
		ThisStdCall(ConstructorPtr, alloc, std::forward<InputArgs>(args)...);
	}
	else
	{
...
template <typename T, const UInt32 DestructorPtr = 0, typename... InputArgs>
void Delete(T* t, InputArgs &&... args)
{
	if constexpr (DestructorPtr)
	{
		ThisStdCall(DestructorPtr, t, std::forward<InputArgs>(args)...);
	}
	FormHeap_Free(t);
}
